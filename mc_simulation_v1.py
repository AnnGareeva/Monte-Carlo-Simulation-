...
def make_monte_carlo_simulation(distribution_params,
                                ...
                                max_iterations=10 ** 6):
    """
    Make Monte-Carlo simulation for generation of distribution of percentile_val% percentile of 10‐days returns.
    1-day returns are generated by stable distribution
    :param distribution_params: ordered list with stable distributions parameters (alpha, beta, gamma, delta)
    :param datasize: size of series of 1-day returns
    :param significance_level: const for Smirnov-Kolmagorov test
    :param min_dstat: dstat valid value for Smirnov-Kolmagorov test
    :param percentile_val: percentile
    :param max_iterations: maximum number of search iterations
    :return: number of iterations it took to pass Smirnov-Kolmagorov test or None if fail to pass test, pval, dstat
    """
    pval, dstat = 0, np.inf
    cnt = 0
    ndays = 10
    ...
    while (pval < significance_level) or (dstat > min_dstat):
        ...
        if cnt >= max_iterations:
            return None, pval, dstat

    return cnt, pval, dstat
...
if __name__ == '__main__':
    ...
    parser.add_argument('-sl', '--significance_level', default=0.5, required=False, type=float,
                        help='probability of rejecting the hypothesis for Smirnov-Kolmagorov test')
    parser.add_argument('-dst', '--min_dstat_value', default=0.01, required=False, type=float,
                        help='dstat valid value for Smirnov-Kolmagorov test')
    ...
    cnt, pval, dstat = make_monte_carlo_simulation(distribution_params,
    ...
    if cnt is not None:
        print(f'Smirnov-Kolmagorov test was passed on iteration {cnt}')
    else:
        print(f'Smirnov-Kolmagorov test was NOT passed')

    print(f'dstat = {dstat}, pvalue = {pval}')
